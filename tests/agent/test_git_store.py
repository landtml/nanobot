...
TRACKED = ["SOUL.md", "USER.md", "memory/observations.md"]
...
class TestLog:
    ...
    def test_message_prefix_skips_unrelated_commits_before_counting_limit(self, git_ready):
        ws = git_ready._workspace
        messages = ["memory: older", "backup: first", "memory: latest", "backup: newest"]
        for i, message in enumerate(messages):
            (ws / "SOUL.md").write_text(f"v{i}", encoding="utf-8")
            git_ready.auto_commit(message)

        commits = git_ready.log(max_entries=2, message_prefix="memory:")

        assert [commit.message for commit in commits] == ["memory: latest", "memory: older"]
...
class TestShowCommitDiff:
    ...
    def test_message_prefix_finds_commit_beyond_unrelated_history_window(self, git_ready):
        ws = git_ready._workspace
        (ws / "SOUL.md").write_text("memory content", encoding="utf-8")
        memory_sha = git_ready.auto_commit("memory: latest")
        for i in range(20):
        ...
        assert result is not None
        commit, diff = result
        assert commit.sha == memory_sha
        assert "memory content" in diff
...
class TestRevert:
    ...
    def test_message_prefix_rejects_unrelated_commit_without_changing_files(self, git_ready):
        ws = git_ready._workspace
        (ws / "SOUL.md").write_text("memory v1", encoding="utf-8")
        git_ready.auto_commit("memory: v1")
        (ws / "SOUL.md").write_text("backup state", encoding="utf-8")
        backup_sha = git_ready.auto_commit("backup: workspace")
        (ws / "SOUL.md").write_text("memory v2", encoding="utf-8")
        latest_sha = git_ready.auto_commit("memory: v2")

        assert git_ready.revert(backup_sha, message_prefix="memory:") is None
        assert (ws / "SOUL.md").read_text(encoding="utf-8") == "memory v2"
        assert git_ready.log()[0].sha == latest_sha


class TestMemoryGitStore:
    def test_memory_tracks_profile_and_observations(self, tmp_path):
        from unittest.mock import MagicMock

        from nanobot.agent.memory import MEMORY_TRACKED_FILES, Memory
        from nanobot.session.manager import SessionManager

        memory = Memory(tmp_path, SessionManager(tmp_path), runtime=MagicMock())
        assert isinstance(memory.git, GitStore)
        assert MEMORY_TRACKED_FILES == ["SOUL.md", "USER.md", "memory/observations.md"]
